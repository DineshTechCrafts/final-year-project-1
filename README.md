...
6. **FAISS Retrieval, Multi-Evidence Reranking, & Synthesis (Stages 8-10):** The resulting database is queried instantly via FAISS (supporting both case-level queries and real-time slice-level single image uploads). While our segmentation model performs well on typical slices (median liver recall of 1.02), we identified a systematic failure mode affecting approximately 2.2% of tumor-positive slices, where the model hallucinates tumor tissue while failing to detect corresponding liver tissue — an anatomically impossible prediction. This occurs predominantly at organ boundary slices (hepatic dome/inferior tip) where the liver's cross-sectional area tapers to near-zero. This finding directly motivated our Stage 9 multi-evidence reranking system, which enforces a strict anatomical consistency rule. The reranker now aggressively drops any candidate slice that predicts a tumor but lacks sufficient liver context (<0.5%), completely removing these visually-deceptive hallucinations from the retrieval results rather than relying solely on learned visual/embedding similarity. Finally, a local LLM agent (Llama 3.1 8B) is implemented (`agents/decision_agent.py`) to synthesize reranked clinical evidence into structured, readable explanations — verified working standalone on real pipeline outputs, though not yet wired into the live web UI.
